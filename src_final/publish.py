...
from pachet import* 
from packettype import PUBLISH
from encode_decode import *


class PublishVariableHeader(VariableHeader):

    def __init__(self, topic_name: str, packet_id: int=None):
        super().__init__()
        if '*' in topic_name:
            raise Exception(" Topic name in the PUBLISH Packet MUST NOT contain wildcard characters.")
...
class PublishPacket(MQTTPacket):
    ...
    def __init__(self, fixed: FixedHeader=None, variable_header: PublishVariableHeader=None, payload=None):
        if fixed is None:
            header = FixedHeader(PUBLISH, 0x00)
        else:
            if fixed.packet_type is not PUBLISH:
                raise Exception("Invalid fixed packet type %s for PublishPacket init")
            header = fixed
