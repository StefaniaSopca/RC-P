from PacketMQTT import *

import encode_decode
class ConnectVariable(MQTTVariable):
...
class ConnectPayload(MQTTPlayload):
    ...
    def to_bytes(self, fixed_header: MQTTFixed, variable_header: ConnectVariable):
        ret = bytearray()

        ret.extend(encode_string(self.id))

        if variable_header.will_flag:
            ret.extend(encode_string(self.will_topic))
            ret.extend(encode_data_with_length(self.will_message))

        if variable_header.username_flag:
            ret.extend(encode_string(self.username))

        if variable_header.password_flag:
            ret.extend(encode_string(self.password))

        return ret


class ConnectPacket(PacketMQTT):
